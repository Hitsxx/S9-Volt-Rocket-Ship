...
int getFixedFreqVoltageValue(int freq)
{
    ...
    if(freq>=675)   // hashrate 14500
        vol_value=880; //                      Hard set all voltages to 8.8v Plan to change voltage per chain at later date based off of hw errors.
...
#endif
                    sprintf(logstr,"Chain[J%d] has no freq in PIC, set default freq=%dM Ooooooh you changed the clock speed... TO THE MOON!\n",i+1,frequency);
                    writeInitLogFile(logstr);
                }
...
//  writeLogFile(logstr);
        ...
        while(1)
        {
            ...
            sprintf(logstr,"read_temp_func Done! Doesn't matter much since I think we are going to want to set that to 100\n");
            writeLogFile(logstr);
            sprintf(logstr,"Why is that you ask? Because we are blasting off to the MOON!\n");
            writeLogFile(logstr);
